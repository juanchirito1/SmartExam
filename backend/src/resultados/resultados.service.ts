...
export class ResultadosService {
  ...
  async calificar(dto: CalificarExamenDto) {
    ...
    const preguntas =
      await this.prisma.pregunta.findMany({
        where: {
    ...
    const numerosRecibidos =
      dto.respuestas.map(
        (respuesta) => respuesta.numero,
      );

    const numerosUnicos =
      new Set(numerosRecibidos);
    ...
    if (
      ...
      throw new BadRequestException(
        'Existen números de pregunta duplicados',
      );
    }
    ...
    const reglas =
      await this.prisma.reglaPuntaje.findMany({
        where: {
    ...
    for (const pregunta of preguntas) {
      ...
      puntajeMaximo +=
        regla.puntajeCorrecta;

      const respuesta =
        respuestaPorNumero.get(
          pregunta.numero,
        );

      if (!respuesta) {
        throw new BadRequestException(
          `Falta la respuesta ${pregunta.numero}`,
        );
      }
    ...
    const resultado =
      await this.prisma.$transaction(
        async (tx) => {
          ...
          await tx.detalleResultado
            .deleteMany({
              where: {
                resultadoId:
                  resultadoGuardado.id,
              },
            });
  ...
  async findByInscripcion(
    ...
    const resultado =
      await this.prisma.resultado.findUnique({
        where: {
          inscripcionId,
        },
    ...
    return resultados.map(
      (resultado, index) => ({
        ...
        alumno: {
          ...
          nombre:
            `${resultado.inscripcion.alumno.nombres} ${resultado.inscripcion.alumno.apellidos}`,
        },
